...
import { 
  ...
  LogOut, Loader2, Activity, Zap, Book, GraduationCap, Quote, Calendar, Clock, Target, Menu, X
} from "lucide-react";
...
function Dashboard({ token, onLogout }) {
  ...
  // Calculate readiness percentage based on current syllabus state (for instant updates)
  const totalTopics = useMemo(() => syllabus.reduce((acc, sub) => acc + sub.topics.length, 0), [syllabus]);
  const totalMastered = useMemo(() => syllabus.reduce((acc, sub) => acc + sub.topics.filter(t => t.userStatus.status === 'mastered').length, 0), [syllabus]);
  
  // Use either the real-time calculated value OR the server value if loading
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans selection:bg-sky-100 flex">
      ...
      <div className="flex-1 flex flex-col md:ml-64 transition-all duration-300 w-full">
        ...
        <main className="max-w-7xl mx-auto p-4 md:p-8 pb-24 w-full">
